(function initializeAirBridgePictureDelay() {
  ...
  class DelayedVideo {
    ...
    positionCanvas() {
      ...
      if (fullscreen && fullscreen !== this.video && fullscreen.contains(this.video) && this.canvas.parentElement !== fullscreen) {
        fullscreen.appendChild(this.canvas);
      } else if (!fullscreen && this.canvas.parentElement !== document.body) {
        document.body.appendChild(this.canvas);
      }
    }
    ...
    fail(reason) {
      ...
      chrome.runtime.sendMessage({ type: "airbridge-status", status: `unsupported: ${reason}` });
      this.stop();
    }
  ...
  class PictureDelayController {
    ...
    configure(enabled, delayMs) {
      ...
      chrome.runtime.sendMessage({
        type: "airbridge-status",
        status: this.enabled ? `active: picture delay ${Math.min(this.delayMs, Core.MAX_BUFFER_MS)} ms` : "disabled",
      });
    }
  ...
  const key = Core.siteKey(location.href);
  ...
  chrome.runtime.onMessage.addListener((message) => {
    if (message.type === "airbridge-sync") controller.configure(message.active, message.offsetMs);
  });
