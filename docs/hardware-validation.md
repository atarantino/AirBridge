...
Discovery on the local Wi-Fi found `Kitchen` as a HomePod Mini running tvOS 26.5 with RAOP pairing reported as `NotNeeded`. TCP port 7000 was reachable and pyatv 0.18.0 discovery passed.

Initial in-memory PCM smoke-test attempts reached RAOP `SETUP` but timed out while the Windows Wi-Fi profile (`BirdHome 2`) was `Public`. After the user changed the trusted home network to `Private`, Kitchen completed transient pairing, RTSP `SETUP`, `RECORD`, event-channel negotiation, and returned its audio/control ports.
...
3. Ensure Kitchen is not part of an unsupported active stereo/multi-room session and restart it if setup times out.
